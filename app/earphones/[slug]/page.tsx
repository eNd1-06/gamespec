...
export default async function EarphoneDetailPage({ params }: Props) {
  ...
  const relatedVs = EARPHONE_VS_PAIRS.filter(([a, b]) => a === ep.slug || b === ep.slug);
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {relatedVs.length > 0 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mt-4">
            <h2 className="text-lg font-bold text-white mb-4">他のイヤホンと比較する</h2>
            <div className="space-y-2">
              {relatedVs.map(([a, b]) => {
                const itemA = earphones.find((x) => x.slug === a)!;
                const itemB = earphones.find((x) => x.slug === b)!;
                return (
                  <Link
                    key={`${a}-${b}`}
                    href={`/earphones/vs/${a}-vs-${b}`}
