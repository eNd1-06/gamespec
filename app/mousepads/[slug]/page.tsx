...
export default async function MousepadDetailPage({ params }: Props) {
  ...
  const relatedVs = MOUSEPAD_VS_PAIRS.filter(([a, b]) => a === pad.slug || b === pad.slug);
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {relatedVs.length > 0 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mt-4">
            <h2 className="text-lg font-bold text-white mb-4">他のマウスパッドと比較する</h2>
            <div className="space-y-2">
              {relatedVs.map(([a, b]) => {
                const itemA = mousepads.find((x) => x.slug === a)!;
                const itemB = mousepads.find((x) => x.slug === b)!;
                return (
                  <Link
                    key={`${a}-${b}`}
                    href={`/mousepads/vs/${a}-vs-${b}`}
