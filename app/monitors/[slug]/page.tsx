...
export default async function MonitorDetailPage({ params }: Props) {
  ...
  const relatedVs = MONITOR_VS_PAIRS.filter(([a, b]) => a === monitor.slug || b === monitor.slug);
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {relatedVs.length > 0 && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 mt-4">
            <h2 className="text-lg font-bold text-white mb-4">他のモニターと比較する</h2>
            <div className="space-y-2">
              {relatedVs.map(([a, b]) => {
                const itemA = monitors.find((x) => x.slug === a)!;
                const itemB = monitors.find((x) => x.slug === b)!;
                return (
                  <Link
                    key={`${a}-${b}`}
                    href={`/monitors/vs/${a}-vs-${b}`}
